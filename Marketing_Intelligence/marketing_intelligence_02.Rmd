...
IE_MI_training <- read_delim("IE_MI_training.txt",";", escape_double = FALSE, trim_ws = TRUE)
...
df <- df %>% select(one_of(c("Dim.1","Dim.2","os","PC1","PC3","PC4","PC5","PC6","PC7","target")))
...
sequence <- seq(1,100)
for (i in sequence){
  tmp = create_model(seed=i,applySVM=F,applyRF = F)
  accuracy[i] <- tmp[[1]]
  f1_measure[i] <- tmp[[2]]
  auc[i] <- tmp[[3]]
...
cat(paste("The seed I should set to obtain the optimal model:", which(auc == max(auc))))
...
# LogReg even better: --> ~0.68; also, very constant for all measures --> best seed = 240 --> AUC = 0.7
```
...
optimalThreshold <- vector()
seed=1
create_model <- function(seed=1){
  set.seed(seed)
  splits <- splitdf(dataframe=df, percentage = 0.8,seed = seed)
  trn = splits$trainset
  tst = splits$testset
  best_model <- best.randomForest(target~., data = trn)
  #Number of trees: 500 --> ntree = 500
  #No. of variables tried at each split: 4 --> mtry = 4

  probs <- predict(best_model, type="prob", newdata = tst,probability=T)  
  predictions <- data.frame(target=tst$target, pred=probs[,2])
  threshold <- getThreshold(predictions)
  #now we predict for our test set
  #diffusion Table
  tbl <- table(predictions$target, predictions$pred > threshold)
...
# MISSING: SCALING THE DATA!!
# MISSING: APPLY KNN ON DATA AGAIN FOR AUC 
...
df <- df %>% select(one_of(c("Dim.1","Dim.2","os","PC3","PC4","PC5","PC6","PC7","target")))
df$target <- as.factor(df$target)
splits <- splitdf(dataframe=df, percentage = 0.7,seed = seed)
train = splits$trainset
require(ROSE)
train = ovun.sample(formula=target~.,data=train,method = "both",p=0.3)$data
test = splits$testset
accuracy <- vector()
f1_measure <- vector()
auc <- vector()
bagging<-function(training,testing,length_divisor=5,iterations=1000,seed=1)
{
  set.seed(seed)
  predictions<-foreach(m=1:iterations,.combine=cbind) %do% {
    training_positions <- sample(nrow(training), size=floor((nrow(training)/length_divisor)))
    train_pos<-1:nrow(training) %in% training_positions
    logistic_regression<-glm(target~.,data=training[train_pos,], family = "binomial")
    return(predict(logistic_regression,newdata=testing))
  }
  auc = apply(predictions,2,function(x){auc(testing$target,x)})
  names = names(head(sort(auc,decreasing = T),20))
  pred = predictions[,names]
  return(rowMeans(pred))
}
sequence = seq(1:10)
for (i in sequence){
  probs = bagging(train,test,seed=i)
  predictions <- data.frame(target=test$target, pred=probs)
  threshold <- getThreshold(predictions)
  auc[i] = auc(predictions$target, predictions$pred)
  accuracy[i] <- mean(test$target==as.factor(if_else(probs>threshold,1,0)))
  tbl = table(test$target,as.factor(if_else(probs>threshold,1,0)))
  f1_measure[i] = (2*(tbl[1,1]))/((2*(tbl[1,1]))+tbl[2,1]+tbl[1,2])
  optimalThreshold[i] = threshold
  print(tbl)
  print(i)
}
auc # always 0.680X --> choose the model that has highest accuracy at the same time
```
