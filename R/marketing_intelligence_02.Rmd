...
create_model <- function(seed=1,applyRF=F,applySVM=F){
  ...
  tst = splits$testset
  if(applyRF==T){
    tbl = randf(trn,cv,tst,seed=seed)
  }
  else if(applySVM==T){
    ...
    tbl = logReg(trn,cv,tst,seed=seed)
  }
  print(tbl)
...
sequence <- seq(1,10)
for (i in sequence){
  tmp = create_model(seed=i,applySVM=T,applyRF = F)
  accuracy[i] <- tmp[[1]]
  f1_measure[i] <- tmp[[2]]
  auc[i] <- tmp[[3]]
...
q3 = qplot(y=auc,x=sequence,xlab="Iteration",ylab="F1 Measure",main="F1 Measure per iteration")+
  geom_line(colour=I("blue"))+
